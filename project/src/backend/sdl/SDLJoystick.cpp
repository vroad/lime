...
namespace lime {
	...
	value Joystick::GetDeviceGUID (int id) {
		
		std::string guid (64, '\0');
		SDL_JoystickGetGUIDString (SDL_JoystickGetDeviceGUID (id), &guid[0], guid.size ());
		return alloc_string_len (guid.c_str (), guid.size ());
